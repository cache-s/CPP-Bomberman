//
// Cube.cpp for cpp_bomber in /home/chazot_a/rendu/cpp_bomberman/tp
// 
// Made by Jordan Chazottes
// Login   <chazot_a@epitech.net>
// 
// Started on  Fri May  8 14:54:16 2015 Jordan Chazottes
// Last update Thu May 28 14:46:38 2015 Jordan Chazottes
//

#include	"Cube.hh"

Cube::Cube()
{
}

Cube::Cube(glm::vec3 position, glm::vec3 rotation, glm::vec3 scale)
{
  _position = position;
  _rotation = rotation;
  _scale = scale;
}

Cube::~Cube()
{
}

bool	Cube::initialize()
{
  _speed = 10.0f;
  if (_texture.load("./textures/grass.tga") == false)
    {
      std::cerr << "Cannot load the cube texture" << std::endl;
      return (false);
    }
  // _geometry.setColor(glm::vec4(1, 0, 0, 1)); // rouge
  // _geometry.pushVertex(glm::vec3(0.5, -0.5, 0.5));
  // _geometry.pushVertex(glm::vec3(0.5, 0.5, 0.5));
  // _geometry.pushVertex(glm::vec3(-0.5, 0.5, 0.5));
  // _geometry.pushVertex(glm::vec3(-0.5, -0.5, 0.5));
  // _geometry.pushUv(glm::vec2(0.0f, 0.0f));
  // _geometry.pushUv(glm::vec2(1.0f, 0.0f));
  // _geometry.pushUv(glm::vec2(1.0f, 1.0f));
  // _geometry.pushUv(glm::vec2(0.0f, 1.0f));

  // _geometry.setColor(glm::vec4(1, 1, 0, 1)); // VERT
  // _geometry.pushVertex(glm::vec3(0.5, -0.5, -0.5));
  // _geometry.pushVertex(glm::vec3(0.5, 0.5, -0.5));
  // _geometry.pushVertex(glm::vec3(-0.5, 0.5, -0.5));
  // _geometry.pushVertex(glm::vec3(-0.5, -0.5, -0.5));
  // _geometry.pushUv(glm::vec2(0.0f, 0.0f));
  // _geometry.pushUv(glm::vec2(1.0f, 0.0f));
  // _geometry.pushUv(glm::vec2(1.0f, 1.0f));
  // _geometry.pushUv(glm::vec2(0.0f, 1.0f));

  // _geometry.setColor(glm::vec4(0, 1, 1, 1)); //BLEU CLAIR
  // _geometry.pushVertex(glm::vec3(0.5, -0.5, -0.5));
  // _geometry.pushVertex(glm::vec3(0.5, 0.5, -0.5));
  // _geometry.pushVertex(glm::vec3(0.5, 0.5, 0.5));
  // _geometry.pushVertex(glm::vec3(0.5, -0.5, 0.5));
  // _geometry.pushUv(glm::vec2(0.0f, 0.0f));
  // _geometry.pushUv(glm::vec2(1.0f, 0.0f));
  // _geometry.pushUv(glm::vec2(1.0f, 1.0f));
  // _geometry.pushUv(glm::vec2(0.0f, 1.0f));

  // _geometry.setColor(glm::vec4(1, 0, 1, 1));
  // _geometry.pushVertex(glm::vec3(-0.5, -0.5, 0.5));
  // _geometry.pushVertex(glm::vec3(-0.5, 0.5, 0.5));
  // _geometry.pushVertex(glm::vec3(-0.5, 0.5, -0.5));
  // _geometry.pushVertex(glm::vec3(-0.5, -0.5, -0.5));
  // _geometry.pushUv(glm::vec2(0.0f, 0.0f));
  // _geometry.pushUv(glm::vec2(1.0f, 0.0f));
  // _geometry.pushUv(glm::vec2(1.0f, 1.0f));
  // _geometry.pushUv(glm::vec2(0.0f, 1.0f));

  _geometry.setColor(glm::vec4(0, 1, 0, 1)); // VERT
  _geometry.pushVertex(glm::vec3(1, 0.5, 1));
  _geometry.pushVertex(glm::vec3(1, 0.5, -1));
  _geometry.pushVertex(glm::vec3(-1, 0.5, -1));
  _geometry.pushVertex(glm::vec3(-1, 0.5, 1));
  _geometry.pushUv(glm::vec2(0.0f, 0.0f));
  _geometry.pushUv(glm::vec2(1.0f, 0.0f));
  _geometry.pushUv(glm::vec2(1.0f, 1.0f));
  _geometry.pushUv(glm::vec2(0.0f, 1.0f));

  // _geometry.setColor(glm::vec4(0, 0, 1, 1));
  // _geometry.pushVertex(glm::vec3(0.5, -0.5, -0.5));
  // _geometry.pushVertex(glm::vec3(0.5, -0.5, 0.5));
  // _geometry.pushVertex(glm::vec3(-0.5, -0.5, 0.5));
  // _geometry.pushVertex(glm::vec3(-0.5, -0.5, -0.5));
  // _geometry.pushUv(glm::vec2(0.0f, 0.0f));
  // _geometry.pushUv(glm::vec2(1.0f, 0.0f));
  // _geometry.pushUv(glm::vec2(1.0f, 1.0f));
  // _geometry.pushUv(glm::vec2(0.0f, 1.0f));
  _geometry.build();
  return (true);
}

void	Cube::update(gdl::Clock const& clock, gdl::Input& input)
{
  // if (input.getKey(SDLK_UP))
  //   translate(glm::vec3(0, 0, -1) * static_cast<float>(clock.getElapsed()) * (_speed*4));
  // if (input.getKey(SDLK_DOWN))
  //   translate(glm::vec3(0, 0, 1) * static_cast<float>(clock.getElapsed()) * (_speed*4));
  // if (input.getKey(SDLK_LEFT))
  //   translate(glm::vec3(-1, 0, 0) * static_cast<float>(clock.getElapsed()) * (_speed*4));
  // if (input.getKey(SDLK_RIGHT))
  //   translate(glm::vec3(1, 0, 0) * static_cast<float>(clock.getElapsed()) * (_speed*4));
}

void	Cube::draw(gdl::AShader& shader, gdl::Clock const&clock)
{
  (void)clock;
  _texture.bind();
  _geometry.draw(shader, getTransformation(), GL_QUADS);
}
